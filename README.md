...
- **リアルタイム同期**: Firestoreによる複数デバイス間でのデータ共有 ✅
  - スマホ・タブレット・PCで同じGmailアカウントでログインするだけで自動同期
  - リアルタイム更新：1つのデバイスで変更すると即座に全デバイスに反映
  - オフライン対応：ネット接続がなくても動作し、再接続時に自動同期
  - 同期ステータス表示：画面右上に接続状態をリアルタイム表示
...
### 同期の仕組み
VoiceTaskは**Cloud Firestore**を使用して、複数のデバイス間でタスクをリアルタイムに同期します。

### セットアップ方法
1. **スマホでログイン**: 同じGmailアカウントでVoiceTaskにログイン
2. **MacBookでログイン**: 同じGmailアカウントでVoiceTaskにログイン
3. **自動同期開始**: それだけで完了！データが自動的に同期されます

### 動作の確認
- **同期インジケーター**: 画面右上に同期状態が表示されます
  - 🟢 **緑の点**: データ同期完了（すべてのデバイスと同期済み）
  - 🟠 **オレンジの点**: 同期中...（データを送受信中）
- **リアルタイム更新**: 
  - スマホでタスクを作成 → MacBookに即座に表示
  - MacBookでタスクを編集 → スマホに即座に反映
  - どちらかでタスクを削除 → 全デバイスから削除

### オフライン対応
- **オフライン時**: インターネット接続がなくても、ローカルキャッシュを使用して動作
- **再接続時**: ネットワークに再接続すると、自動的に変更をアップロード・ダウンロード
- **競合解決**: 複数デバイスで同時に編集した場合、最後の変更が優先されます

### トラブルシューティング
**同期されない場合:**
1. **ログイン確認**: 両方のデバイスで同じGmailアカウントでログインしているか確認
2. **ネットワーク確認**: インターネット接続を確認
3. **ブラウザ更新**: ページをリロードしてみる
4. **ログアウト・再ログイン**: 一度ログアウトして再ログイン

**同期インジケーターが表示されない場合:**
- Firebase認証が完了していない可能性があります
- `.env.production`のFirebase設定を確認してください
