...
### 音声入力の設定（初回のみ）
1. **Safariブラウザ**でアプリを開く（Chrome等は非対応）
2. マイクボタンをタップ
3. 「マイクへのアクセスを許可しますか？」と表示されたら**「許可」**をタップ
4. これで音声入力が使えるようになります

### 音声入力が動作しない場合
1. **設定** → **Safari** → **カメラとマイク** → 「許可」に設定
2. アプリをリロード
3. マイクボタンを再度タップ
...
2. **カレンダー表示切替**: 「週表示」または「月表示」を選択
3. **日付選択**: カレンダーから予定を登録したい日付をタップ
   - 週表示: 横スクロールで7日間表示
   - 月表示: グリッドで1ヶ月全体を表示
4. **タスク追加**: ボイスメモエリアで音声入力または手入力
5. **確認して登録**: 選択した日付を確認して「この日付に追加」をタップ
6. **タスク管理**: チェックボックスで完了、展開して詳細表示・削除
7. **通知設定**: 重要タスクは自動で通知ON、手動でも切替可能
...
- **月次カレンダービュー**（2025-10-29）
  - 月単位でタスクを俯瞰できるカレンダーグリッドを実装
  - 週表示/月表示の切り替えボタンを追加
  - カレンダーの主な機能:
    * 7x6グリッドレイアウトで1ヶ月全体を表示
    * 各日付にタスク数を表示（例：「3件」）
    * 前月/次月への移動ボタン
    * 「今日」ボタンで当日に素早く移動
    * 選択中の日付をグラデーションでハイライト
    * 今日の日付を紫枠で強調表示
    * タスクがある日をピンク背景で視覚化
    * 日曜日は赤、土曜日は青で表示
    * 当月外の日付は薄く表示
...
- **iPhone音声認識の信頼性向上**（2025-10-29）
  - iOS Safari専用の最適化を実装
  - `webkitSpeechRecognition`を優先的に使用
  - マイク権限の事前確認機能を追加
  ...
  - 視覚的フィードバックの強化:
    ...
    * アニメーションリングで録音中を視覚化
    * 成功/エラーメッセージボックス
    * ボタンヒント表示（「タップ」「話してください」）
  - エラーハンドリングの改善:
    * 権限拒否、音声未検出、ネットワークエラーなど
    * 各エラーに対する具体的な対処方法を表示
  - 100msのディレイでiOS安定性を向上
